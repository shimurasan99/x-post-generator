...
ENDING_NOTES = [
    "自分はこういう時ほど、ニュースの見出しより裏にある資金の流れを見たくなります。",
    "短期の値動きで焦る気持ちは分かるけど、今どの材料が意識されているかを先に整理したいです。",
    "強気にも弱気にも寄りすぎず、いくつかのシナリオを持っておく方が気持ち的にも楽ですね。",
    "売買を急ぐ前に、根拠、時間軸、撤退ラインをメモするだけでもかなり落ち着けます。",
    "個人的には、価格予想よりも市場が何を怖がっているかを見る方が面白いです。",
]

MARKET_HOOKS = [
    "こういう材料が出ると、ついチャートを開きたくなりますw",
    "ここ、投資家の本音がかなり出そうです。",
    "見出しだけで分かった気になるのはちょっと危ないかもです。",
    "短期勢と長期勢でかなり見方が割れそうです。",
    "SNSの熱量と実際の需給は、いったん分けて見たいですね。",
]

TOPICAL_HOOKS = [
    "正直、SNSがざわつく理由は分かりますw",
    "こういう揉め方って、見ている側も感情を持っていかれますね。",
    "表のニュースだけで判断すると、ちょっと浅くなりそうです。",
    "野次馬で終わらせるには、少しもったいない話です。",
    "エンタメっぽく見えて、実は信用の話でもあります。",
]
...
BAIT_HOOKS = [
    "これ、スルーしてる人けっこう危ないと思う。",
    "正直、ここに違和感を持てるかで差が出ます。",
    "きれいごと抜きで、かなり生々しい話です。",
    "これをただのニュースで終わらせるのは浅いです。",
    "分かる人には分かる、けっこう嫌な空気です。",
]

LENGTH_SUPPLEMENTS = {
    "話題ニュース": (
        "こういう話題は、誰かを一方的に叩くより、発言の時系列、一次情報、SNSで広がった理由を分けて見たいです。"
        "信用が揺れる瞬間って、情報の扱い方にかなり人間性が出る気がします。"
    ),
    "default": (
        "こういう材料は、見出しだけで終わらせず、市場の反応、出来高、金利や需給まで見たいです。"
        "こういう材料ほど、焦って決めつけるより一回メモして整理した方が落ち着けます。"
    ),
}
...
TEMPLATES = [
    "{genre}の「{theme}」、自分はここをかなり気にしています。理由は、相場が何に反応しているかで空気が変わるから。{context}だからこそ、値動きだけで判断すると少し怖いです。{note}",
    "「{theme}」は、個人的にかなり見ておきたい材料です。上がる下がるを決め打ちするより、どの時間軸の人が動いているかを見たい。{context}なので、焦らず背景を分けて考えたいですね。{note}",
    "{genre}の「{theme}」を見て、少し空気が変わった感じがします。強そうな材料に見えても、もう価格に入っている可能性もあるので油断はしたくないです。{note}",
    "「{theme}」で気になるのは、価格の方向より市場がどこを怖がっているかです。{genre}は{context}なので、自分なら期待シナリオと崩れる条件をセットで見ます。{note}",
    "投資って、早くニュースを知るより、そのニュースをどう感じてどう整理するかが大事だと思います。{genre}の「{theme}」も、金利、需給、リスク許容度まで見たいところ。{note}",
]

POP_TEMPLATES = [
    "{emoji}{genre}の「{theme}」、正直ちょっと目が離せないですw {hook} 値動きだけ見るより、何に市場が反応しているかを追う方が面白い。{context}なので、自分なら短期のノイズと本命材料を分けて見ます。{note}",
    "{emoji}「{theme}」で相場が少しざわついていますね。こういう時って、つい強気か弱気かで見たくなるけど、自分はまず参加者が何を怖がっているかを見たいです。{hook} 見出しだけで判断するのは少し怖い。",
    "{emoji}{genre}界隈、また面白い材料が出てきましたねw テーマは「{theme}」。SNSの熱量に引っ張られる気持ちは分かるけど、出来高、金利、需給、時系列はセットで見たい。{personal} {note}",
    "{emoji}正直、「{theme}」は投稿ネタとしてもかなり強いですw でも煽りだけで終わると薄いので、なぜ今この話題が注目されているのかまで見たい。{context}という前提を置くと、相場の見え方が少し変わります。",
    "{emoji}「{theme}」を見て、すぐ売買判断に飛ぶのは少し早い気がします。まずは市場が何を好感して、何を嫌がっているかを整理したい。{hook} {genre}は材料の受け止め方で空気が変わるので、ここは要チェックです。",
]

TOPICAL_TEMPLATES = [
    "{emoji}「{theme}」これ、ただの話題ニュースとして流すには濃すぎますねw {hook} {context}なので、誰が正しいかを決めつけるより「なぜここまで広がったのか」を見たくなります。{note}",
    "{emoji}{genre}で今気になっているのは「{theme}」の裏側です。表では揉め事っぽく見えても、信用、影響力、お金の流れが見えることもあります。{hook} 個人的には、こういう話ほど一次情報を見たいです。",
    "{emoji}この「{theme}」、SNS向きの燃え方をしていますねw 見ているとつい感情が動くけど、そこで一回止まりたい。{context}だからこそ、発言の時系列と一次情報はセットで見たいです。{personal}",
    "{emoji}投資垢でも「{theme}」はスルーしにくい話題だと思います。相場もビジネスも、最後は信用で空気が変わるから。誰かを叩くより、何が信用を揺らしたのかを見る方が学びが多い気がします。{hook}",
    "{emoji}正直、「{theme}」みたいな話題は伸びやすいですw でも煽りだけだと薄いので、信用、資金、人間関係、SNS拡散の4点で見たい。自分はこういうニュースほど、熱量より時系列を大事にしたいです。{note}",
]

BAIT_MARKET_TEMPLATES = [
    "{emoji}これ、かなり大事です。\n\n「{theme}」をただの相場ニュースだと思ってる人、多分けっこう見落としてます。\n\n自分が怖いのは値動きそのものより、みんなが同じ方向を見始めた時の空気感。{hook}\n\nこういう時ほど、焦った人から判断が雑になります。",
    "{emoji}正直、「{theme}」は軽く見ない方がいいと思ってます。\n\n上がる下がるの話じゃなくて、市場の空気が変わるきっかけになりやすいから。\n\n{context}という前提があるので、ここを見ずに雰囲気だけで乗るのはかなり怖いですw",
    "{emoji}「{theme}」を見て何も感じないなら、相場の見方ちょっと危ないかもです。\n\nニュース自体より大事なのは、その後に誰が焦って、誰が冷静に見ているか。\n\n{hook}\n\n自分ならここは、価格より参加者の温度感を見ます。",
    "{emoji}{genre}界隈、また荒れそうな材料が出てきましたね。\n\nこういう時に一番怖いのは、SNSのテンションに飲まれて自分の判断が消えること。\n\n{personal}\n\n煽りに見えるかもですが、こういう局面ほど一回冷静になった方が残りやすいです。",
    "{emoji}この「{theme}」、正直かなり投稿向きのネタですw\n\nでも本当に見たいのは派手な見出しじゃなくて、裏で何が警戒されているか。\n\n{hook}\n\n相場って、みんなが油断したタイミングほど嫌な動きをするので。",
]

BAIT_TOPICAL_TEMPLATES = [
    "{emoji}これ、正直かなりエグいです。\n\n「{theme}」をただの揉め事として見てる人、多分もったいない。\n\n表では感情論に見えるけど、裏では信用、影響力、お金、人間関係が全部絡んでるように見えます。\n\n{hook}",
    "{emoji}「{theme}」で一番怖いのは、誰が正しいかよりも“信用が崩れる瞬間”が見えてしまうことです。\n\nSNSって、火がつくと一気に人の見方が変わる。\n\nこれ、ビジネスでも投資でもかなり重い話だと思います。",
    "{emoji}この件、野次馬で見てる人多いと思うけど、個人的にはかなり学びがあります。\n\n「{theme}」みたいな話題って、感情が先に広がって事実確認が後回しになりがち。\n\n{hook}\n\nだからこそ、時系列と一次情報は見たいです。",
    "{emoji}正直、「{theme}」は伸びる要素がそろいすぎてますw\n\n有名人、対立、信用、お金、SNS拡散。\n\nそりゃみんな反応しますよね。\n\nただ、自分は誰かを叩くより、なぜここまで人が反応するのかを見たいです。",
    "{emoji}これを見て何も感じない人、かなりメンタル強いですw\n\n「{theme}」って、ただのスキャンダルじゃなくて、信用がどう作られてどう壊れるかの話に見えます。\n\n{personal}\n\nこういう話題ほど、表だけ見て判断すると危ないです。",
]
...
def generate_posts(theme: str, genre: str, tone: str = "ポップ強め") -> list[str]:
    ...
    notes = ENDING_NOTES[:]
    emojis = POP_EMOJIS[:]
    personals = PERSONAL_REACTIONS[:]
    if tone == "落ち着きめ":
        templates = TEMPLATES[:]
        hooks = MARKET_HOOKS[:]
    elif tone == "煽り強め" and genre == "話題ニュース":
        templates = BAIT_TOPICAL_TEMPLATES[:]
        hooks = BAIT_HOOKS[:]
    elif tone == "煽り強め":
        templates = BAIT_MARKET_TEMPLATES[:]
        hooks = BAIT_HOOKS[:]
    elif genre == "話題ニュース":
    ...
    for index in range(5):
        post = templates[index].format(
            ...
            personal=personals[index % len(personals)],
        )
